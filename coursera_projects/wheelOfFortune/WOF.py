...
class WOFPlayer():
    def __init__(self, name):
        ...
        self.prizeMoney = 0
        self.prizes = []
    def addMoney(self, amt): self.prizeMoney += amt
    def goBankrupt(self): self.prizeMoney = 0
    def addPrize(self, prize): self.prizes.append(prize)
    def __str__(self): return '{} (${})'.format(self.name, self.prizeMoney)
...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        s_input = input("Guess a letter, phrase, or type 'exit' or 'pass': ")
        if s_input == 'exit': exit()
        elif s_input == 'pass': pass
        else: return s_input
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        if random.randint(1, 10) > self.difficulty: return True
        else: return False
    def getPossibleLetters(self, guessed):
        guessed_list = [x for x in LETTERS if x not in guessed]
        if VOWEL_COST > super().prizeMoney: 
            guessed_list = [x for x in guessed_list if x not in VOWELS]
            return guessed_list
        else: return guessed_list
    def getMove(self, category, obscuredPhrase, guessed):
        result = self.getPossibleLetters(guessed)
        if len(result) < 1: return 'pass'
        if self.smartCoinFlip(): 
            for ch in self.SORTED_FREQUENCIES[::-1]: 
                if ch in result: return ch 
        else: return random.choice(result)
